...
Future<List<Reports>> fetchReports() async {
  ...
  Uri url = Uri.parse('$IP_URL/userReport/dashboard/$current_user_id/reports');
...
Future<List<AssignTask>> fetchAssignedReports() async {
  ...
  Uri url = Uri.parse('$IP_URL/actionTeam/dashboard/$current_user_id/fetchAssignedTasks');
  final response = await http.get(url);
  if (response.statusCode == 200) {
    List<dynamic> jsonResponse = jsonDecode(response.body) as List<dynamic>;
...
Future<void> postapprovedActionReport(int? user_report_id, int? action_report_id ) async {
  ...
  Future<bool> postReport(String image, String sublocation, String incidentSubType, String description, DateTime date, String risklevel) async {
    ...
    Uri url = Uri.parse('$IP_URL/userReport/dashboard/$current_user_id/MakeReport');
    try {
  ...
  Future<void> uploadReportWithImage(XFile? imageFile, String sublocation, String incidentSubType, String description, DateTime date, String risklevel) async {
     SharedPreferences prefs = await SharedPreferences.getInstance();
    current_user_id = prefs.getString("user_id");
    print('current_user_id:$current_user_id');

  if (imageFile != null) {
    Uri apiUri = Uri.parse('$IP_URL/userReport/dashboard/$current_user_id/MakeReport');

    var request = http.MultipartRequest('POST', apiUri);

    // Add other form data
    request.fields['report_description'] = description;
    request.fields['date_time'] = date.toLocal().toIso8601String().split(".")[0]; // Replace with actual date time
    request.fields['sub_location_id'] = sublocation; // Replace with actual sub location ID
    request.fields['incident_subtype_id'] = incidentSubType; // Replace with actual incident subtype ID
    request.fields['incident_criticality_id'] = risklevel; // Replace with actual incident criticality ID
  ...
  throw Exception('Failed to load all Reports');
    ...
    current_user_id = prefs.getString("user_id");
    print('current_user_id:$current_user_id');
    Uri url = Uri.parse('$IP_URL/actionTeam/dashboard/$current_user_id/MakeActionReport');
    try {
      final http.Response response = await http.post(
        url,
        headers: {"Content-Type": "application/json"},
...
Future<String?> uploadImage(File imageFile) async {
  try {
    var request = http.MultipartRequest('POST', Uri.parse('$IP_URL/api/image/upload'));
